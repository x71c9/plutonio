...
export class SourceFile {
  ...
  public nodes = new Map<string, types.NodeWrap>();
  public imports: types.Import[] = [];
  ...
  private _resolve_nodes() {
    ...
    // Interface
    const interface_nodes = utils.get_nested_of_type<ts.InterfaceDeclaration>(
      this.source,
    ...
    // Type
    const type_nodes = utils.get_nested_of_type<ts.InterfaceDeclaration>(
      this.source,
  ...
  private _resolve_imports() {
    ...
    const import_declarations: ts.ImportDeclaration[] =
      utils.get_nested_of_type(this.source, ts.SyntaxKind.ImportDeclaration);
